"""Tests for reporting what an image-based meta-analysis actually used.
...
asserts a transform rule: NiMARE decides what it can convert and what it drops,
and the report relays that.
"""
...
def _fit_and_describe(studyset, estimator, dropped_maps=None):
    """Run the workflow the way the runner does, then read the outcome off it."""
...
def test_a_supplied_map_is_not_reported_as_derived(write_map):
    """A study that uploaded a z-map should not look like one we transformed."""
    studyset = _studyset(write_map, {"s1": (["Z map"], None)})

    _, report = _fit_and_describe(studyset, Stouffers(aggressive_mask=True))

    (analysis,) = report.analyses
    assert analysis.included
    assert analysis.supplied == {"z"}
    assert analysis.derived == frozenset()
    assert analysis.reason == "supplied the required maps"
...
def test_an_analysis_nimare_cannot_transform_is_excluded_with_a_reason(write_map):
    ...
    Paired with a study that can be used, because the point is that the run
    succeeds and reports the exclusion rather than failing.
    """
...
def test_nothing_usable_is_described_from_the_submission(write_map):
    ...
    NiMARE raises before one exists, so the account has to come from what was
    submitted instead.
    """
...
def test_an_analysis_with_the_map_but_no_metadata_is_explained(write_map):
    ...
    The map is there, so the reason cannot be a missing map -- the report has to
    reach for the metadata requirement to explain the exclusion.
    """
...
def test_tsv_has_one_row_per_analysis(write_map):
    studyset = _studyset(
        write_map, {"good": (["Z map"], None), "bad": (["univariate-beta map"], None)}
    )

    _, report = _fit_and_describe(studyset, Stouffers(aggressive_mask=True))
    lines = report.to_tsv().strip().splitlines()

    assert len(lines) == 3  # header plus two analyses
    assert lines[0].startswith("study_id\t")
    assert [line.split("\t")[4] for line in lines[1:]] == ["true", "false"]
...
def test_varcope_is_derived_from_beta_and_t(write_map):
    ...
    Asserted through NiMARE rather than against a rule table, so if upstream
    changes what it can derive this test tracks it instead of contradicting it.
    """
...
def test_supplied_maps_are_reported_per_analysis(write_map):
    ...
    NiMARE's image table has a column per type across the whole studyset, and
    an absent path reads as NaN, which is truthy. Taking truthiness would
    report every analysis as holding every type -- and produce the nonsense
    "could not produce z from ... z".
    """
    studyset = _studyset(
        write_map,
        {
