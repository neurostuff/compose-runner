...
def test_a_derived_map_is_reported_as_derived(write_map):
    ...
    studyset = _studyset(
        write_map, {"s1": (["T map"], 20), "s2": (["T map"], 20), "s3": (["T map"], 20)}
    )
...
def test_partial_coverage_still_yields_a_usable_studyset(write_map):
    ...
    assert report.n_studies_included == 3
    assert {a.study_id for a in report.included} == {
        "supplied",
        "supplied2",
        "derivable",
    }
...
def test_summary_names_the_excluded_analyses(write_map):
    ...
    assert "Used 3 of 4 analyses" in summary
...
def test_dropped_maps_are_carried_into_the_report(write_map):
    ...
    studyset = _studyset(
        write_map,
        {
            "s1": (["univariate-beta map"], None),
            "s2": (["univariate-beta map"], None),
            "s3": (["univariate-beta map"], None),
        },
    )
...
def test_varcope_is_derived_from_beta_and_t(write_map):
    ...
    studyset = _studyset(
        write_map,
        {
            "s1": (["univariate-beta map", "T map"], None),
            "s2": (["univariate-beta map", "T map"], None),
            "s3": (["univariate-beta map", "T map"], None),
        },
    )
...
def test_supplied_maps_are_reported_per_analysis(write_map):
    ...
    studyset = _studyset(
        write_map,
        {
            ...
            "has_z": (["Z map"], None),
            # Two more usable studies: rc4 refuses a fit that reaches the
            # estimator with fewer than three analyses once the jackknife runs.
            "has_z2": (["Z map"], None),
            "has_z3": (["Z map"], None),
        },
    )
